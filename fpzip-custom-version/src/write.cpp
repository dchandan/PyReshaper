...
)
{
  ...
  for (unsigned i = 0; i < nf; i++) {
    ...
    switch (bits) {
      ...
      compress_case(32);
      default:
        fprintf(stderr, "fpzip: precision %d not supported\n", bits);
        abort();
        break;
    }
    data += nx * ny * nz;
...
unsigned fpzip_memory_write_py(
  float*       buffer, // pointer to compressed data
  unsigned    size,   // size of allocated storage
  const float* data,   // array to write
  const int*  prec,   // per field bits of precision
  int         dp,     // double precision array if nonzero
  unsigned    nx,     // number of x samples
  unsigned    ny,     // number of y samples
...
{ 
  if(!fpzip_memory_write((void*)buffer,size,(const void*)data,prec,dp,nx,ny,nz,nf)) {
    fprintf(stderr,"fpzip: cannot write to memory\n");
    abort();
  }
  return 1;
}
...
void
fpzip_memory_write_f(
  void*     buffer, // pointer to compressed data
  const int*  size,   // size of allocated storage
  const void* data,   // array to write
  const int*  prec,   // per field bits of precision
...
{
  if(!fpzip_memory_write(buffer,*size,data,prec,*dp,*nx,*ny,*nz,*nf)) {
    fprintf(stderr,"fpzip: cannot write to memory\n");
    abort();
  }
}
void
fpzip_memory_write_f_(
  void*     buffer, // pointer to compressed data
  const int*  size,   // size of allocated storage
  const void* data,   // array to write
  const int*  prec,   // per field bits of precision
...
{
  fpzip_memory_write_f(buffer,size,data,prec,dp,nx,ny,nz,nf);
}
void
fpzip_file_write_f(
...
)
{
  ...
  if (!file) {
    fprintf(stderr, "fpzip: cannot create file %s\n", path);
    abort();
  }
  if (!fpzip_file_write(file, data, prec, *dp, *nx, *ny, *nz, *nf)) {
    fprintf(stderr, "fpzip: cannot write file %s\n", path);
    abort();
  }
  fclose(file);
}
