...
int main(int argc, char* argv[])
{
  ...
  unsigned outbytes = fpzip_memory_write(buffer, size * sizeof(*data), data, prec, dp, dim[0], dim[1], dim[2], dim[3]);
   newbuffer=fpzip_memory_write_py(buffer, size * sizeof(*data), data, prec, dp, dim[0], dim[1], dim[2], dim[3]);
  ...
  t = now();
  if (!fpzip_memory_read((unsigned char*)newbuffer, copy, prec, dp, dim[0], dim[1], dim[2], dim[3])) {
    fprintf(stderr, "decompression failed\n");
    return EXIT_FAILURE;
  }
  t = now() - t;
