...
class Reshaper(object):
    ...
    def print_diagnostics(self):
        ...
        byte_count_str = _pprint_dictionary('BYTE COUNTS', total_bytes)
...
#        opt.PreFill = False
        ...
        self._messenger.print_once('netCDF4python options set', vlevel=2)

        # Open all of the input files
        self._timer.start('Open Input Files')
        self._input_files = []
        for filename in self._specifier.input_file_list:
            self._input_files.append(nc4.Dataset(filename, "r"))
    ...
    def _sort_input_files_by_time(self):
        ...
        for ifile in self._input_files:
            time_values.append(ifile.variables['time'][:])
    ...
    def _sort_variables(self):
        ...
        for vname in  self._time_series_variables:
            ...
            vsizes[vname] = numpy.size(v) * numpy.dtype(v.dtype).itemsize

        from operator import itemgetter
        vsorted = sorted(vsizes.items(),key=itemgetter(1),reverse=True) 
        #print vsorted
        self._time_series_variables = []
        for item in vsorted:
            self._time_series_variables.append(item[0])

        #print self._time_series_variables
        # Debug output
    ...
    def convert(self, output_limit=0):
        ...
        # Synching output before convert
        self._messenger.sync()
        ...
        print common_dims
        common_atts = {}
        for name in ref_infile.ncattrs():
            common_atts[name] = getattr(ref_infile,name)
        ...
        for out_name in tsv_names_loc:
            ...
            out_file = nc4.Dataset(out_filename, 'w', format=self.format)
            out_file.set_fill_off()
            ...
            self._messenger.print_all('start: Create Time-Invariant Metadata ' + out_name, vlevel=4)
            ...
            for name in self._time_invariant_metadata:
                ...
                for att_name in in_var.ncattrs():
                    if att_name == '_FillValue':
                        fillvalue = getattr(in_var,att_name)
                        break
                out_var = out_file.createVariable(name,
                    in_var.dtype, in_var.dimensions,
                    zlib = set_zlib, 
                    complevel = self.compression_level,
                    fill_value = fillvalue)
                for att_name in in_var.ncattrs():
                    if att_name != '_FillValue':
                        setattr(out_var, att_name, getattr(in_var,att_name))
            self._timer.stop('Create Time-Invariant Metadata')
            self._messenger.print_all('end: Create Time-Invariant Metadata ' + out_name, vlevel=4)

            # Create the time-variant metadata variables
            self._messenger.print_all('start: Create Time-Variant Metadata ' + out_name, vlevel=4)
            ...
            self._messenger.print_all('end: Create Time-Variant Metadata ' + out_name, vlevel=4)

            # Create the time-series variable itself
            self._messenger.print_all('start: Create Time-Series Variables ' + out_name, vlevel=4)
            ...
            self._messenger.print_all('end: Create Time-Series Variables ' + out_name, vlevel=4)
...
class MultiSpecReshaper(object):
    ...
    def convert(self, output_limit=0):
        ...
        for spec_name in self._specifiers:
            ...
            rshpr._messenger.sync()

            self._messenger.print_once('--- Finished converting Specifier: ' \
                + spec_name, vlevel=0)
            self._messenger.print_once('', vlevel=0)
    ...
    def print_diagnostics(self):
        ...
        for name in self._specifiers:
            ...
            counts_str = _pprint_dictionary('BYTE COUNTS', counts)
